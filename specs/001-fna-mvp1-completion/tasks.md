...
- [x] T047 Add an SDD spec-conformance stage to .github/workflows/claude-pr-review.yml — implemented, reverted from PR #4 (workflow-file/main mismatch broke every review on that PR), then landed as its own follow-up PR after #4 merged. Correction to the note left in PR #4: opening a fresh PR does NOT sidestep the anti-tamper check when the PR's own purpose is to modify claude-pr-review.yml — the check compares file content against main regardless of event type (opened vs synchronize), so this follow-up PR's own review is itself skipped. That's expected and fine for a small, manually-verified config change; every PR after this one merges gets the SDD-conformance stage. Stage reads the active specs/<slug>/ directory and flags diff behaviour with no FR backing it, unimplemented in-scope FRs, contract docs describing behaviour the code doesn't have, and tasks.md checkboxes marked done that aren't; widened claude_args --allowedTools to include Glob/Grep
