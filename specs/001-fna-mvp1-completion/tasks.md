...
- [ ] T047 Add an SDD spec-conformance stage to .github/workflows/claude-pr-review.yml — implemented, then reverted from this PR: `anthropics/claude-code-action@v1` refuses to exchange its OIDC token for an app token when the triggering workflow file on the PR branch differs from the copy on `main` ("Workflow validation failed. The workflow file must exist and have identical content to the version on the repository's default branch"), which silently stopped `claude-pr-review` from running on every push to this PR once the file diverged. Land this as its own follow-up PR after #4 merges — a fresh PR's workflow file will trivially match `main` at open time and pass validation normally. The intended stage (read the active specs/<slug>/ directory and flag diff behaviour with no FR backing it, unimplemented in-scope FRs, contract docs describing behaviour the code doesn't have, and tasks.md checkboxes marked done that aren't; widen claude_args --allowedTools to include Glob/Grep) is unchanged — only the landing sequence changes
